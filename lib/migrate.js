...
const reverseModels = function (sequelize, models) {
  ...
  for (let model in models) {
    ...
    for (let column in attributes) {
      if (column === "id") id_type = attributes[column]["type"];
      ...
      case "D":
        {
          ...
          if (df.path[1] === "schema") {
            ...
            if (df.path.length === 3) {
              if (df.lhs.field === "id") break;
              if (df.lhs.field === "createdAt") break;
              if (df.lhs.field === "updatedAt") break;
              // drop column
              actions.push({
...
const getPartialMigration = function (actions) {
  let propertyToStr = (obj, attrName) => {
    let vals = [];
    for (let k in obj) {
      if (k === "seqType") {
  ...
  let getAttributes = (attrs) => {
    ...
    let newAttrs = {
      ...
      created_at: {
        ...
        defaultValue: { internal: true, value: "Sequelize.fn('NOW')" },
      },
      updated_at: {
        allowNull: false,
        field: "updated_at",
        ...
        defaultValue: {
          internal: true,
          value: "Sequelize.literal('NOW() ON UPDATE NOW()')",
        },
      },
    };
    ...
    for (let attrName in newAttrs) {
      ...
      ret.push(
        `      "${attrName}": ${propertyToStr(newAttrs[attrName], attrName)}`
      );
    }
    return " { \n" + ret.join(", \n") + "\n     }";
  };
...
`;
  ...
  let filename = path.join(
    migrationsDir,
    new Date().toISOString().match(/\d+/g).join("") +
      (name != "" ? `-${name}` : "") +
      ".js"
  );
